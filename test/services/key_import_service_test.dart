...
void main() {
  ...
  const _testEd25519Pem = '''-----BEGIN OPENSSH PRIVATE KEY-----
...
-----END OPENSSH PRIVATE KEY-----''';
  ...
  group('KeyImportService.fromPem', () {
    test('parses ed25519 key and derives public key string', () {
      final key = KeyImportService.fromPem('test_key', _testEd25519Pem);

      expect(key.name, 'test_key');
      expect(key.privateKeyPem, _testEd25519Pem);
      expect(key.publicKey, isNotEmpty);
      expect(key.publicKey, startsWith('ssh-ed25519 '));
    });

    test('generates a unique id per import', () {
      final a = KeyImportService.fromPem('a', _testEd25519Pem);
      final b = KeyImportService.fromPem('b', _testEd25519Pem);
